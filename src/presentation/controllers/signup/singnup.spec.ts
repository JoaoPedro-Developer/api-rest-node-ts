...
describe('SingnUp Controller', () => {
  ...
  test('Should return 400 if password confirmation is not equal a password', () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  test('Should return 400 if and invalid e-mail is provided', () => {
  ...
  test('Should return 500 email validator throws', () => {
    const { sut, emailValidatorStub } = makeSut()
  ...
  test('Should addAccount if not valid values', () => {
  ...
  test('Should return 500 add account throws', () => {
    const { sut, addAccountStub } = makeSut()
